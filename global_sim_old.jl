...
ic_filepath = "longer_null_hypothesis.jl" # without parameterizations etc.
# qs_filepath = "long_null_hypothesis.jl"
qs_filepath = "even_longer_null_hypothesis.jl"
...
closures = (vertical_diffusivity, convective_adjustment, biharmonic_viscosity) # , gent_mcwilliams_diffusivity) # (horizontal_diffusivity, vertical_diffusivity, convective_adjustment, biharmonic_viscosity, gent_mcwilliams_diffusivity)
model = HydrostaticFreeSurfaceModel(grid=grid,
    free_surface=free_surface,
    momentum_advection= VectorInvariant(), # WENO5(vector_invariant=VelocityStencil()),
